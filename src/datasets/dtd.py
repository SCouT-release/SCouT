...
def _load_huggingface_split(split, location):
    try:
        ...
        raise ImportError(
            "DTD loading from Hugging Face requires the `datasets` package. "
            "Install it with `pip install datasets`."
        ) from exc

    cache_dir = os.path.join(os.path.expanduser(location), "huggingface")
    return load_dataset("tanganke/dtd", split=split, cache_dir=cache_dir)
...
class DTD:
    def __init__(
        ...
        print("Loading tanganke/dtd from Hugging Face.")
