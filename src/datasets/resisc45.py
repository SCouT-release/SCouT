...
class RESISC45Dataset(VisionClassificationDataset):
    ...
    * https://doi.org/10.1109/jproc.2017.2675998
    ...
    def __init__(
        ...
        def is_in_split(path: str) -> bool:
            return os.path.basename(path) in valid_fns
...
def _load_huggingface_split(split, location):
    try:
        ...
        raise ImportError(
            "RESISC45 loading from Hugging Face requires the `datasets` package. "
            "Install it with `pip install datasets`."
        ) from exc

    cache_dir = os.path.join(os.path.expanduser(location), "huggingface")
    return load_dataset("tanganke/resisc45", split=split, cache_dir=cache_dir)
...
class RESISC45:
    def __init__(self,
        ...
        if self._local_data_exists(location):
            ...
            print("Loading tanganke/resisc45 from Hugging Face.")
    ...
    @staticmethod
    def _local_data_exists(location):
        data_root = os.path.join(location, RESISC45Dataset.directory)
        split_root = os.path.join(location, "resisc45")
        return (
            os.path.isdir(data_root)
            and os.path.isfile(os.path.join(split_root, "resisc45-train.txt"))
            and os.path.isfile(os.path.join(split_root, "resisc45-test.txt"))
        )
