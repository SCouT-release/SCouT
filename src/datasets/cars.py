...
def _load_huggingface_split(split, location):
    try:
        ...
        raise ImportError(
            "Cars loading from Hugging Face requires the `datasets` package. "
            "Install it with `pip install datasets`."
        ) from exc

    cache_dir = os.path.join(os.path.expanduser(location), "huggingface")
    return load_dataset("tanganke/stanford_cars", split=split, cache_dir=cache_dir)
...
class Cars:
    def __init__(
        ...
        print("Loading tanganke/stanford_cars from Hugging Face.")
