...
def _set_mha_fastpath_enabled(enabled):
...
class LinearizedModel(nn.Module):
    ...
    def __init__(self, model: nn.Module, init_model: nn.Module = None) -> None:
        ...
        self.param_names = tuple(name for name, _ in model.named_parameters())
        ...
        assert len(self.param_names) == len(self.params)
    ...
    def __call__(self, x) -> torch.Tensor:
        ...
        # PyTorch fused attention kernels do not implement forward AD used by jvp.
        mha_fastpath_enabled = _get_mha_fastpath_enabled()
        if mha_fastpath_enabled is not None:
            _set_mha_fastpath_enabled(False)
        try:
            ...
            if mha_fastpath_enabled is not None:
                _set_mha_fastpath_enabled(mha_fastpath_enabled)
        return out + dp
