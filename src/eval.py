...
# Eval single task with the test dataset.
def eval_single_dataset(image_encoder, dataset_name, args):
    ...
    with torch.no_grad():
        top1, correct, total_loss, n = 0.0, 0.0, 0.0, 0.0
    ...
    metrics = {"top1": top1, "loss": loss}
    print(
        f"Done evaluating on {dataset_name}. "
        f"Accuracy: {100*top1:.2f}%\tLoss: {loss:.4f}"
    )

    return metrics

# Loop over args.eval_datasets, optionally appending args.control_dataset, and calls eval_single_dataset for each one.
def evaluate(image_encoder, args):
    ...
    for dataset_name in eval_datasets:
        ...
        per_dataset_results[dataset_name + ":loss"] = results["loss"]

    return per_dataset_results

# Return the Task Arithmetic accuracy coef_info[dataset_name + ":top1"] (pretrained_checkpoint, task vectors and scaling_coef needed),
# also the averaged noramlized accuracy coef_info["avg_normalized_top1"] and averaged accuracy coef_info["avg_top1"]
def evaluate_task_vector_at_coef(
    ...
    coef_info["avg_loss"] = np.mean(
        [coef_info[dataset + ":loss"] for dataset in args.eval_datasets]
    )

    return coef_info

# Sweeps scaling_coef from 0.0 to args.max_coef using args.n_eval_points.
def evaluate_task_vector(
    task_vector, pretrained_checkpoint, args, posthoc_linearization=False
):
    info = {}
    for scaling_coef in np.linspace(0.05, args.max_coef, args.n_eval_points):
